...
create index if not exists occasional_sale_reservations_user_id_idx
  on public.occasional_sale_reservations (user_id);

update public.occasional_sale_reservations reservation
set user_id = profile.id
from public.profiles profile
where reservation.user_id is null
  and lower(btrim(coalesce(reservation.client_email, ''))) = lower(btrim(coalesce(profile.email, '')))
...
using (
  user_id = auth.uid()
  or lower(btrim(coalesce(client_email, ''))) = lower(btrim(coalesce(auth.jwt()->>'email', '')))
);

drop function if exists public.create_occasional_sale_reservation(text, integer, text, text, text, text);
drop function if exists public.create_occasional_sale_reservation(text, integer, text, text, text, text, text);
...
begin
  ...
  if p_item_id = 'reform-hens'
    and nullif(btrim(coalesce(p_client_address, '')), '') is null then
    raise exception 'occasional_sale_address_required';
  end if;
...
grant execute on function public.create_occasional_sale_reservation(text, integer, text, text, text, text, text)
