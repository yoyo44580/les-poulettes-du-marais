...
export default function KennelContractModal({ booking, profile, contract, amount, dailyRate, isAdmin, onClose, onSign }) {
  ...
  const stay = contract?.snapshot?.stay || {
    ...
    arrivalTime: String(booking?.arrival_time || "09:00").slice(0, 5),
    departureTime: String(booking?.departure_time || "18:00").slice(0, 5),
    amount,
    dailyRate,
  };
    ...
    <div className="contract-modal" role="dialog" aria-modal="true" aria-label="Contrat de pension canine">
      <div className="contract-modal__panel">
        ...
        <div className="kennel-contract-document" ref={documentRef}>
          ...
          <section className="contract-section"><h3>5. Durée du séjour</h3><p><b>Date d'arrivée :</b> {formatDate(stay.startDate)} à {stay.arrivalTime || "09:00"}<br /><b>Date de départ :</b> {formatDate(stay.endDate)} à {stay.departureTime || "18:00"}</p><p className="contract-check-line">{declarationMark("startedDayDue")} J'accepte que toute journée entamée soit due</p></section>
